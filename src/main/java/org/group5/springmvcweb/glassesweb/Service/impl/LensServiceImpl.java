...
public class LensServiceImpl implements LensService {
    ...
    public LensServiceImpl(LensRepository lensRepository,
                           ReadyMadeGlassesRepository readyMadeGlassesRepository) {
    ...
    public Lens updateLens(Integer id, UpdateLensRequest request) {
        ...
        if (request.getLensType() != null) {
            // Chap nhan tat ca gia tri lensType hop le
    ...
    public void deleteLens(Integer id) {
        if (!lensRepository.existsById(id))
            throw new RuntimeException("Trong kinh khong ton tai!");
        if (readyMadeGlassesRepository.existsByLensId(id))
            throw new RuntimeException("Khong the xoa! Trong kinh nay dang duoc gan voi san pham kinh co san.");
        try {
            lensRepository.deleteById(id);
        } catch (Exception e) {
            throw new RuntimeException("Khong the xoa! Trong kinh nay dang duoc su dung trong don hang hoac thiet ke.");
        }
    }
    ...
    private void validateLensData(String name, String lensType, Boolean colorChange, String lensSize,
        ...
        // lensType va lensSize khong validate cung - lay tu dropdown admin
