...
public class GlassesDesignController {
    ...
    // --------------------------------------------------
    // Staff/Operation xem chi tiết thiết kế kính theo myGlassesId
    // --------------------------------------------------
    @GetMapping("/api/my-glasses/{id}")
    @PreAuthorize("hasAnyRole('USER','STAFF','ADMIN','OPERATION','SHIPPER')")
    public ResponseEntity<ApiResponse<MyGlassesResponse>> getMyGlassesById(
            @PathVariable Integer id) {

        return ResponseEntity.ok(ApiResponse.ok(
                glassesDesignService.getMyGlassesById(id)));
    }
