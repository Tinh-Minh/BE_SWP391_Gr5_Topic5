...
public class StaffController {
    ...
    // Tu choi don hang (khi thong tin sai hoac het hang)
    @PutMapping("/orders/{orderId}/reject")
    public ResponseEntity<Map<String, String>> rejectOrder(
            @PathVariable Integer orderId,
            @RequestBody Map<String, String> body) {
        String reason = body.getOrDefault("reason", "Thong tin don hang khong hop le");
        fulfillmentService.rejectOrder(orderId, reason);
        return ResponseEntity.ok(Map.of("message", "Da tu choi don hang #" + orderId + "! Ly do: " + reason));
    }
